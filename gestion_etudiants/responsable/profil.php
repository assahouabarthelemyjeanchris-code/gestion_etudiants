...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT motdepasse FROM responsablefiliere WHERE id_responsablefiliere = ?");
    ...
    if ($user && password_verify($current_password, $user['motdepasse'])) {
        ...
        if (!empty($new_username)) {
            $upd = $conn->prepare("UPDATE responsablefiliere SET nomutilisateur = ? WHERE id_responsablefiliere = ?");
        ...
        if (!empty($new_password)) {
            if ($new_password === $confirm_password) {
                ...
                $upd = $conn->prepare("UPDATE responsablefiliere SET motdepasse = ? WHERE id_responsablefiliere = ?");
