...
if (isset($_POST['email'])) {
    ...
    try {
        ...
        if ($res_admin) {
            ...
            // Verifions si la table contient AU MOINS un email pour voir le format
            $check_format = $conn->query("SELECT email FROM administrateur LIMIT 1")->fetch();
            if($check_format) {
                echo "Note : Un email trouvé en BDD ressemble à ceci : [" . $check_format['email'] . "]<br>";
            } else {
                echo "ALERTE : La table 'administrateur' est totalement VIDE ou la colonne email est vide.<br>";
            }
        }
