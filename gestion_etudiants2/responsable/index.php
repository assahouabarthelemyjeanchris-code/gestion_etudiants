        ...
        <div class="row g-5">
            ...
            <div class="col-lg-4">
                <div class="mb-5">
                    ...
                    <h2 class="fw-800 tracking-tight mt-2"><?= htmlspecialchars($nom_departement) ?></h2>
            ...
            <div class="col-lg-8">
                <div class="glass-card">
                    <h3 class="fw-800 mb-4">Fiches d'inscription reçues</h3>
                            ...
                            ?>
                                <div class="notif-card <?= $is_unread ? 'notif-unread' : '' ?>">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <h6 class="fw-bold m-0 text-uppercase text-secondary">
                                    ...
                                    <p class="text-dark bg-light p-2 rounded small mb-3"><?= htmlspecialchars($notif['message']) ?></p>

                                    <?php if($etudiant_data): ?>
                                        <div class="fiche-inscription shadow-sm">
                                            <div class="fw-bold mb-2 text-dark d-flex justify-content-between">
                                                <span><i class="fa-solid fa-id-card me-1"></i> FICHE D'INSCRIPTION REMPLIE</span>
                                                <span class="badge bg-warning text-dark"><?= htmlspecialchars($etudiant_data['statut_inscription']) ?></span>
                                            </div>
                                            
                                            <div class="section-title">Identité de l'étudiant</div>
                                            <div class="row g-2">
                                                <div class="col-md-6"><strong>Nom :</strong> <?= htmlspecialchars($etudiant_data['nom']) ?></div>
                                                <div class="col-md-6"><strong>Prénom :</strong> <?= htmlspecialchars($etudiant_data['prenom']) ?></div>
                                                <div class="col-md-6"><strong>Sexe :</strong> <?= htmlspecialchars($etudiant_data['sexe']) ?></div>
                                                <div class="col-md-6"><strong>Né(e) le :</strong> <?= date('d/m/Y', strtotime($etudiant_data['date_naissance'])) ?> à <?= htmlspecialchars($etudiant_data['lieu_naissance']) ?></div>
                                                <div class="col-md-6"><strong>Nationalité :</strong> <?= htmlspecialchars($etudiant_data['nationalite']) ?></div>
                                                <div class="col-md-6"><strong>Email :</strong> <?= htmlspecialchars($etudiant_data['email']) ?></div>
                                                <div class="col-md-6"><strong>Contact :</strong> <?= htmlspecialchars($etudiant_data['contact_etudiant']) ?></div>
                                                <div class="col-md-6"><strong>Adresse :</strong> <?= htmlspecialchars($etudiant_data['adresse']) ?></div>
                                            </div>

                                            <div class="section-title">Cursus Académique Demandé</div>
                                            <div class="row g-2">
                                                <div class="col-md-6"><strong>Filière :</strong> <?= htmlspecialchars($etudiant_data['nom_filiere']) ?></div>
                                                <div class="col-md-6"><strong>Niveau d'entrée :</strong> <?= htmlspecialchars($etudiant_data['niveau']) ?></div>
                                                <div class="col-md-6"><strong>Etablissement d'origine :</strong> <?= htmlspecialchars($etudiant_data['etablissement_origine'] ?? 'Néant') ?></div>
                                                <div class="col-md-6"><strong>Série du BAC :</strong> <?= htmlspecialchars($etudiant_data['serie_diplome'] ?? '-') ?> (<?= htmlspecialchars($etudiant_data['points_diplome'] ?? '0') ?> pts)</div>
                                            </div>

                                            <div class="section-title">Contacts des Parents & Tuteurs</div>
                                            <div class="row g-2">
                                                <div class="col-md-6"><strong>Père :</strong> <?= htmlspecialchars($etudiant_data['nom_prenom_pere'] ?? 'Néant') ?> (<?= htmlspecialchars($etudiant_data['telephone_pere'] ?? '-') ?>)</div>
                                                <div class="col-md-6"><strong>Mère :</strong> <?= htmlspecialchars($etudiant_data['nom_prenom_mere'] ?? 'Néant') ?> (<?= htmlspecialchars($etudiant_data['telephone_mere'] ?? '-') ?>)</div>
                                                <div class="col-md-12"><strong>Tuteur légal :</strong> <?= htmlspecialchars($etudiant_data['tuteur_legal']) ?> | Tél: <?= htmlspecialchars($etudiant_data['telephone_tuteur']) ?></div>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="d-flex justify-content-end gap-2 mt-3 align-items-center">
                                        <?php if($is_unread): ?>
                                            <a href="req_action_notif.php?action=lu&id=<?= $notif['id_notification'] ?>" class="btn btn-sm btn-outline-secondary py-1" style="border-radius:8px;">Marquer lu</a>
                                        <?php elseif($notif['statut_lecture'] == 1): ?>
                                            <a href="req_action_notif.php?action=historique&id=<?= $notif['id_notification'] ?>" class="btn btn-sm btn-link text-muted small text-decoration-none">Déplacer à l'historique</a>
                                        <?php endif; ?>
                                        
                                        <?php if($etudiant_data && $etudiant_data['statut_inscription'] !== 'Validé définitivement'): ?>
                                            <form action="req_valider_etudiant.php" method="POST" class="d-inline m-0">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-dark-modern py-1.5 px-3" style="font-size:0.85rem;">
                                                    <i class="fa-solid fa-user-check me-1"></i> VALIDER LA CANDIDATURE
                                                </button>
                                            </form>
                                        <?php elseif($etudiant_data): ?>
                                            <span class="text-success small fw-bold"><i class="fa-solid fa-circle-check"></i> Étudiant validé et visible</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
